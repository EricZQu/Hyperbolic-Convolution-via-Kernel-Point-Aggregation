...
class Lorentz(LorentzOri):
    ...
    def split(self, x, k, dim = -1):
        dx = x.size(dim) - 1
        if dx % k != 0:
            raise ValueError(
                "Split dim not dividable."
            )
        y = torch.stack(x.narrow(dim, 1, dx).split(k, dim), dim)
        dim0 = dim + 1 if dim != -1 else -1
        y0 = torch.sqrt((y**2).sum(dim0, keepdim=True) + self.k)
        y = torch.concat((y0, y), dim0)
        return y
